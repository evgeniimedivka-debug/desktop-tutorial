...
Бот хранит в Cloudflare KV (`CHAT_HISTORY`) последние 100 сообщений чата
(~50 обменов) и передаёт их в Anthropic API как контекст — учитывает,
...
обрезаются (без ошибок).

Больше история — дороже каждый запрос к Anthropic API (весь контекст
пересчитывается заново каждый раз), это влияет на счёт по факту
использования. Лимит — константа `HISTORY_LIMIT` в `src/index.js`.

Посмотреть/очистить память вручную:
...
- [ ] Бот помнит предыдущие сообщения в рамках последних ~50 обменов.
